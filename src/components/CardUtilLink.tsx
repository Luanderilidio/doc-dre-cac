...
export default function CardUtilLink() {
  const [openCard, setOpenCard] = useState(false);
  const [served, setServed] = useState(true)

  return (
    <div className="bg-blue-200 p-4 border-l-4 border-b-4 border-blue-800 rounded-xl shadow-md flex flex-col gap-3 font-Montserrat">
      <div className="flex items-center justify-between text-blue-900 gap-1 relative">
        ...
        <div className="text-xs flex flex-col items-center justify-center bg-blue-600 text-white p-[2px] rounded-sm shadow-sm absolute -top-1 right-0">
          <p className="text-[.7rem] leading-none">Inicia em</p>
          <p className="font-black leading-none">12/03/2025</p>
        </div>
      </div>
      <h1 className="font-bold text-2xl text-blue-600  font-Montserrat leading-none mt-1">
        Texto super importante aqui
      </h1>
      <p className="leading-tight font-semibold text-blue-900">
      ...
      >
        <ExpandMoreIcon className="text-blue-600 p-0" />
      </ExpandMore>
      <Collapse
        in={openCard}
        timeout="auto"
        unmountOnExit
        className="font-Inter text-blue-800 "
      >
