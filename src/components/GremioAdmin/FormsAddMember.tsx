...
export default function FormsAddMember({ gremio_id }: FormsAddMemberProps) {
    ...
    const handleDataGetRoles = async () => {
        try {
            ...
            const rolesNotActive = rolesOriginal.filter(role => !response.data.includes(role))
            ...
            <div className="grid grid-cols-12 gap-4 border-red-500">
                <form
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Autocomplete
                                ...
                                value={
                                    field.value
                                        ? students.find((student) => student.id === field.value) ?? null
                                        : null
                                }
                                ...
                                renderOption={(props, option) => (
                                    <li {...props} key={option.id}>
                                        <div className="flex gap-3 items-start justify-start">
                                            ...
                                            <div>
                                                <strong className="text-sm">{option.name.split(" ")[0]}</strong>
                                                <div className="text-[.7rem]">
                                                    <p>Matrícula <span className="font-semibold">{option.registration}</span></p>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                    ...
                    <Tooltip title="Adicionar Membro" arrow>
                        <Button type="submit" className="col-span-2" variant="outlined" color="primary">
                ...
                <div className="col-span-12 flex flex-col gap-1">
                    {members?.map((member) => (
                        <CardMemberGremio key={member.id} member={member} onDelete={handleDeleteMember} />
                    ))}
