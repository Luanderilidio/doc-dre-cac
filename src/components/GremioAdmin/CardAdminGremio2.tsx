...
export default function AdminGremio() {
    ...
    const [viewFormsAddMembers, setViewFormsAddMembers] = useState<boolean>(false);
    ...
    const filteredData = data.filter((gremio) => {
        const nameMatch = filters.name === "" || gremio.name.toLowerCase().includes(filters.name.toLowerCase());
        const cityMatch = filters.city === "" || gremio.school.city.toLowerCase().includes(filters.city.toLowerCase());
        const schoolMatch = filters.school === "" || gremio.school.name.toLowerCase().includes(filters.school.toLowerCase());
        const teacherMatch = filters.interlocutor === "" || gremio.interlocutor.name.toLowerCase().includes(filters.interlocutor.toLowerCase());
        const statusMatch =
            filters.status === "all" ||
        ...
        <div className="h-full grid grid-cols-12 gap-5  ">
            <div className="col-span-12 border grid grid-cols-12 gap-3 bg-gray-100/60 p-4 rounded-lg">
                ...
                <Autocomplete
                    ...
                    renderInput={(params) => <TextField {...params} label="Nome do Grêmio" />}
                />
                ...
                <Autocomplete
                    ...
                    renderInput={(params) => <TextField {...params} label="Interlocutor" />}
                />
                ...
                <Button onClick={() => setViewFormsAddGremio(true)} className="col-span-1" variant="outlined" color="primary">
            ...
            <div className="col-span-12 grid grid-cols-2 gap-5">
                {viewFormsAddGremio && (
                    <div className="bg-gray-100/60 p-4 rounded-lg border">
                        <div className="flex items-center justify-between">
                            ...
                            <IconButton aria-label="" onClick={() => setViewFormsAddGremio(false)}>
                ...
                {viewFormsAddMembers && (
                    <div className="bg-gray-100/60 p-4 rounded-lg border">
                        <div className="flex items-center justify-between">
                            <p className="col-span-12 text-xl font-bold mb-4">Adicionar Membros</p>
                            <IconButton aria-label="" onClick={() => setViewFormsAddMembers(false)}>
                                <CloseIcon />
                            </IconButton>
