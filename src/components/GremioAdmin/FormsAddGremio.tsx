...
export default function FormsAddGremio({
  setIdGremio,
  setViewFormsAddMembers,
  gremioEditData,
}: FormsAddGremioProps) {
  ...
  //   console.log("gremioEditData", gremioEditData);
  ...
  const {
    ...
    defaultValues: gremioEditData
      ? {
          name: gremioEditData.name,
          status: gremioEditData.status,
          url_profile: gremioEditData.url_profile,
          url_folder: gremioEditData.url_folder,
          validity_date: gremioEditData.validity_date,
          approval_date: gremioEditData.approval_date,
          school_id: gremioEditData.school.id,
          interlocutor_id: gremioEditData.interlocutor.id,
        }
      : defaultValues,
  });
  ...
  const handleDataGetSchools = async () => {
    try {
      ...
      setSchools([...(gremioEditData?.school ? [gremioEditData.school] : [])]);
    }
  };
  ...
  const handleDataGetInterlocutors = async () => {
    try {
      ...
      setInterlocutors([...(gremioEditData?.interlocutor ? [gremioEditData.interlocutor] : [])]);
    }
  };
      ...
      <Controller
        ...
        render={({ field }) => (
          <Autocomplete
            ...
            isOptionEqualToValue={(option, value) => option.id === value?.id}
            value={
              interlocutors.find((i) => i.id === selectedInterlocutorId) || null
            }
            onChange={(_, newValue) => {
              field.onChange(newValue?.id || "");
            }}
