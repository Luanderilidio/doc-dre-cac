import { AppBar, Avatar, AvatarGroup, Button, Dialog, IconButton, Toolbar, Typography } from "@mui/material";
    ...
    return (<div className="shadow-md rounded-xl font-Inter">
        ...
        <div className="rounded-b-md border p-4 flex flex-col gap-3 bg-gray-200/30">
            <div className="flex items-center justify-start gap-2 text-[.7rem] font-bold">
                <h1 className="flex items-center justify-start"> <PlaceIcon sx={{ fontSize: 13 }} /> {data.school.city}</h1>
                <h1 className="flex items-center justify-start gap-1"><HomeWorkIcon sx={{ fontSize: 13 }} /> {data.school.name}</h1>
            </div>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center justify-start gap-2">
                    <Avatar variant="rounded" alt="Remy Sharp" src={faker.image.avatar()} sx={{ width: 30, height: 30 }} />
                    <div className="flex flex-col">
                        <h1 className="text-xs font-semibold">{data.interlocutor.name.split(" ")[0]}</h1>
                        <h1 className="text-[.6rem] font-semibold">{data.interlocutor.contact}</h1>
                    </div>
                </div>
                <Button onClick={openViewDialog} size="small" startIcon={<RemoveRedEyeIcon />}>
                    Administrar
                </Button>
            </div>
        </div>
        ...
        >
            ...
            <div className="grid grid-cols-12 gap-5 p-5">
                ...
                <div className="col-span-4 bg-gray-100/60 p-4 rounded-lg border ">
                    ...
                    <FormsAddGremio setIdGremio={function (_id: string): void {
