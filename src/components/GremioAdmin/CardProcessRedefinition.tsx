...
import { Collapse, ListItemIcon, IconButton } from "@mui/material";
import { ProcessRedefinitionStages, ProcessRedefinitionWithStages, Stage } from "./SchemaGremioAdmin";
...
export default function CardProcessRedefinition({ data }: Props) {
  ...
  const getProcessRefefinitionStages = async () => {
    ...
    try {
      ...
      const apiMapByStage = new Map(response.data.map(obj => [obj.stage, obj]));

      // Itera no array base e substitui se encontrar o mesmo 'stage' na API
      const mergedArray = stagesBase.map(baseObj => {
        // Cast forçado para o tipo do enum (Stage)
        const stageKey = baseObj.stage as Stage;

        if (apiMapByStage.has(stageKey)) {
          return apiMapByStage.get(stageKey)!;
        }
        return baseObj;
      });
  ...
  return (
    <div className="p-4 rounded-lg bg-gray-300/30 border">
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        ...
        <List>
          {redefinitionStages?.map((stage, index) => (
            <div  >

              <ListItem alignItems="flex-start">
                <ListItemAvatar>
                  <div className={`font-bold text-3xl ${stage.status ? '' : 'text-red-500'}`}>
                    {index + 1}
                  </div>
                </ListItemAvatar>
                <ListItemText
                    ...
                    <div className="">
                      ...
                      {stage.status && (
                        <div className="flex items-center gap-1">
                          ...
                          <p className="text-xs"> {moment(stage.started_at).format("MM/YY")} até {moment(stage.finished_at).format("MM/YY")}</p>
                        </div>
                      )}
                    </div>
                  }
